...
raw_rss_feed_links_list = [
    line
    for line in news_sources_raw_list
...
parsed_list = [feedparser.parse(rss_link).entries for rss_link in rss_feed_links_list]
...
total_num_of_articles = sum([len(feed_dicts) for feed_dicts in extracted_dicts_list])
...
for feed_dicts in extracted_dicts_list:
    ...
    for article_dict in feed_dicts:
        ...
        company_checker_dict = {
            "{}-{}".format(company_name, ticker): company_name.lower()
            in article_title.lower()
            or ticker.lower() in article_title.lower()
            for company_name, ticker in zip(company_names_list, company_tickers_list)
        }
        if sum(list(company_checker_dict.values())) > 0:
            ...
            mentioned_companies_list = [
                key.split("-") for key, value in company_checker_dict.items() if value
            ]
            for company_name, ticker in mentioned_companies_list:
                ...
                try:
                    ...
                    name_index = company_dict["Company_Name"].index(company_name)
                    ticker_index = company_dict["Company_Ticker"].index(ticker)
...
assert np.all(np.array(num_mentioned_list) >= np.array(num_descr_list))
...
company_df = (
    pd.DataFrame(company_dict)
    .sort_values(by="Num_Mentioned", ascending=False)
...
if __name__ == "__main__":
    ...
    print("---------------------------")
    print(
        "The total number of articles worked with is: {}".format(total_num_of_articles),
        end="\n\n",
    )
    print(
    ...
    print(
        "The total number of articles with tags: {}".format(num_articles_with_tag),
        end="\n\n",
    )
    print(
